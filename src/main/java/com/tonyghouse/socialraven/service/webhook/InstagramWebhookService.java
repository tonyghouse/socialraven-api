...
public class InstagramWebhookService {
    ...
    public String verifySubscription(String mode, String token, String challenge) {
        if (!"subscribe".equals(mode) || !StringUtils.hasText(challenge)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Instagram webhook verification request");
        }
        if (!StringUtils.hasText(verifyToken) || !verifyToken.equals(token)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Invalid Instagram webhook verify token");
        }
        return challenge;
    }
    ...
    public void processEvent(String rawPayload, String signatureHeader) {
        ...
        try {
            ...
            log.error("Failed to process Instagram webhook payload", exception);
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Instagram webhook payload");
        }
    }

    private void handleEntry(JsonNode entry) {
        String providerUserId = textValue(entry.path("id"));
    ...
    private Set<String> extractProviderPostIds(JsonNode entry) {
        ...
        for (JsonNode change : changes) {
            ...
            addIfText(providerPostIds, value.path("media_id"));
            addIfText(providerPostIds, value.path("post_id"));
            addIfText(providerPostIds, value.path("id"));
            addIfText(providerPostIds, value.path("media").path("id"));
        }
        return providerPostIds;
    }
    ...
    private void validateSignature(String rawPayload, String signatureHeader) {
        if (!StringUtils.hasText(signatureHeader) || !signatureHeader.startsWith("sha256=")) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Missing Instagram webhook signature");
        }
        if (!StringUtils.hasText(appSecret)) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Instagram app secret not configured");
        }

        try {
            Mac mac = Mac.getInstance("HmacSHA256");
            mac.init(new SecretKeySpec(appSecret.getBytes(StandardCharsets.UTF_8), "HmacSHA256"));
            byte[] expected = hex(mac.doFinal(rawPayload.getBytes(StandardCharsets.UTF_8))).getBytes(StandardCharsets.UTF_8);
            ...
            if (!java.security.MessageDigest.isEqual(expected, provided)) {
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid Instagram webhook signature");
            }
        } catch (ResponseStatusException exception) {
            throw exception;
        } catch (Exception exception) {
